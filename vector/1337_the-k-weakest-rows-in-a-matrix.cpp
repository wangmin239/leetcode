...
public:
    vector<int> kWeakestRows(vector<vector<int>>& mat, int k) {
        vector<pair<int, int>> soldierRow;
        auto getSoldierCnt = [&](const vector<int>& vec, const int row) -> void {
            
            int soldierCnt = 0;
            for (auto soldier : vec) {
                if (soldier == 1) {
                    soldierCnt++;
                } else {
                    break;
            ...
            soldierRow.emplace_back(soldierCnt, row);
        };
        ...
        for (int row = 0; row < mat.size(); row++) {
            getSoldierCnt(mat[row], row);
        }
        
        sort(soldierRow.begin(), soldierRow.end(), cmp);
        
        vector<int> ans;
        for (int i = 0; i < k; i++) {
            ans.push_back(soldierRow[i].second);
        }
