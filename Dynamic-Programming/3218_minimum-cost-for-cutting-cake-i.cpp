...
public:
    int minimumCost(int m, int n, vector<int>& horizontalCut, vector<int>& verticalCut) {
        int cnt[]{ 1,1 }, ans{};
        ranges::sort(horizontalCut, greater{}), ranges::sort(verticalCut, greater{});
        vector<int>::iterator it[]{ horizontalCut.begin(), verticalCut.begin() };
        vector<int>::iterator end[]{ horizontalCut.end(), verticalCut.end() };
        while (it[0] != end[0] || it[1] != end[1]) {
            int pos = ranges::max(0, 1, {}, [&](int i) { return it[i] == end[i] ? 0 : *it[i]; });
            ans += *it[pos]++ * cnt[!pos];
            cnt[pos]++;
        }
        return ans;
